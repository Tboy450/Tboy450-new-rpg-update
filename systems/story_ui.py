...
def draw_help_controls_overlay(
    screen,
    *,
    font_large,
    font_small,
    ...
    """Draw the beginner help panel for desktop and Android players.

    Beginner note:
        This helper only draws text. `main.py` decides when the panel opens,
        closes, and which inputs should be ignored while it is visible.
    """
    _draw_dim_overlay(screen, screen_width, screen_height, 190)

    panel = pygame.Rect(110, 82, screen_width - 220, screen_height - 164)
    pygame.draw.rect(screen, ui_bg, panel, border_radius=14)
    pygame.draw.rect(screen, (255, 215, 0), panel, 3, border_radius=14)

    title = font_large.render("HELP / CONTROLS", True, (255, 215, 0))
    screen.blit(title, (panel.centerx - title.get_width() // 2, panel.y + 24))

    if android_mode:
        close_text = "Tap CLOSE HELP to return."
    else:
        close_text = "Press ESC, ENTER, SPACE, or click to return."
    close_label = font_tiny.render(close_text, True, (210, 210, 220))
    screen.blit(close_label, (panel.centerx - close_label.get_width() // 2, panel.y + 66))

    help_rows = [
        ("Move", "Desktop: arrow keys or WASD.", "Android: touch the arrow pad."),
        ("Use", "ENTER confirms menus and talks.", "SPACE/USE interacts with doors and people."),
        ("Pause", "ESC opens this menu on desktop.", "Android: MENU opens the same menu."),
        ("Battle", "Pick an action when it is your turn.", "ACTIONS/HIDE keeps the battle buttons tidy."),
        ("Progress", "LOG tracks quests and town errands.", "MAP shows visited areas; Inventory equips gear."),
    ]

    y = panel.y + 118
    for heading, line_one, line_two in help_rows:
        heading_text = font_small.render(heading, True, text_color)
        screen.blit(heading_text, (panel.x + 42, y))

        detail_one = font_tiny.render(line_one, True, (235, 235, 225))
        detail_two = font_tiny.render(line_two, True, (210, 220, 230))
        screen.blit(detail_one, (panel.x + 190, y + 1))
        screen.blit(detail_two, (panel.x + 190, y + 27))
        y += 78

    footer = font_tiny.render(
        "Tip: Save before testing risky fights, then Load from this menu if needed.",
        True,
        (255, 235, 160),
    )
    screen.blit(footer, (panel.centerx - footer.get_width() // 2, panel.bottom - 44))
